...
void rechercherCategorie()
{
    ...
    printf("Entrez l'ID de la cat�gorie � rechercher : ");
    scanf("%d", &id);
    while (fread(&c, sizeof(Categorie), 1, f))
    {
        if (c.id == id)
    ...
    if (!trouve) printf("Cat�gorie non trouvee.\n");
    fclose(f);
}
...
void modifierCategorie()
{
    ...
    if (trouve) printf("Modification r�ussie.\n");
}
...
void supprimerCategorie()
{
    ...
    remove("DATABASE/CATEGORIES.dat");
    rename("DATABASE/temp.dat", "DATABASE/CATEGORIES.dat");
    if (trouve) printf("Suppression r�ussie.\n");
}
